...
### Summary document (auto-generated by each condor job)

Each condor job runs `summarize_outputs.py` as its last step, so when the outputs
return you already have a single Markdown index of every figure and CSV. Open the
markdown directly, or the HTML rendering for a styled view:

```
sf_2d_<YEAR>/figures_sf_2d/summary.md
sf_2d_<YEAR>/figures_sf_2d/summary.html      # browser → Cmd/Ctrl-P → Save as PDF

parking_vbf_<YEAR>/figures_parking_vbf/summary.md
parking_vbf_<YEAR>/figures_parking_vbf/summary.html
```

The script tries `pandoc` first (any backend) and falls back to a pure-Python
markdown→HTML rendering with embedded CSS and page-break hints when pandoc isn't
installed. If `python-markdown` isn't available either, only the markdown file is
produced. To regenerate locally (e.g. after editing the figures):
