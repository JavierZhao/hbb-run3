...
class MuonTriggerSFProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        numerator_count = 0
        # Build numerator from the OR of all triggers configured for this year
        trig_list = trigger_dict_periods.get(self.year, [])
        or_mask_trigs = None
        for trig in trig_list:
            tfield = trig[4:] if trig.startswith('HLT_') else trig
            # Only consider triggers present in the NanoAOD HLT fields
            if tfield in denominator_events.HLT.fields:
                mask = denominator_events.HLT[tfield]
                or_mask_trigs = mask if or_mask_trigs is None else (or_mask_trigs | mask)

        if or_mask_trigs is not None:
            numerator_events = denominator_events[or_mask_trigs]
            ...
            # No configured triggers found in this file's HLT branches
            print(f"Warning: No configured triggers for year '{self.year}' found in HLT paths for dataset {dataset}! Skipping numerator fill.")
        ...
        # also record OR of configured triggers (in denominator_events) if available
        if 'or_mask_trigs' in locals() and or_mask_trigs is not None:
            output['selected_counts'][key_or] += int(ak.count_nonzero(or_mask_trigs))
        else:
            output['selected_counts'][key_or] += 0
        return output
...
file_dict_periods = {'2022': {'ttbar': "TTtoLNu2Q", 'MuonData': "Muon_Run2022C"},
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Scale Factor Analysis")
    ...
    # Validate that the year has configured triggers
    if YEAR not in trigger_dict_periods:
        raise ValueError(f"Year '{YEAR}' not found in trigger_dict_periods. Available years: {list(trigger_dict_periods.keys())}")

    trig_list = trigger_dict_periods[YEAR]
    if len(trig_list) == 0:
        raise ValueError(f"No triggers configured for year {YEAR}.")
    ...
    fileset = {}
    # Default behavior: read per-prod_mode JSON files under infiles/{YEAR}/{YEAR}_{prod_mode}.json
    for prod_mode, file_key in prod_modes_map.items():
        print(f'infiles/{YEAR}')
    ...
    # Prefilter fileset to only include reachable files
    def check_file(path, timeout=60):
        try:
            # Open file and attempt to access the Events tree; be robust to
            # minor variations in how uproot reports keys.
            with uproot.open(path, timeout=timeout) as f:
                try:
                    # Preferred: direct access — will raise if missing
                    _ = f["Events"]
                    return True
                except Exception:
                    # Fallback: scan keys strings for any appearance of 'Events'
                    keys = [str(k) for k in f.keys()]
                    evt_keys = [k for k in keys if "Events" in k]
                    if evt_keys:
                        try:
                            _ = f[evt_keys[0]]
                            return True
                            ...
                            pass
                    # Helpful debug: show a short sample of keys when skipping
                    sample = keys[:10]
                    print(f"  Skipping {path}: no Events tree. Keys sample: {sample}")
                    return False
        except Exception as e:
            print(f"  Unreachable {path}: {e}")
            return False

    # prefilter fileset
    filtered_fileset = {}
    for ds, flist in fileset.items():
        good = []
        for f in flist:
            if check_file(f, timeout=60):
                good.append(f)
        if good:
            filtered_fileset[ds] = good
        else:
            print(f"Dataset {ds} has no reachable files after prefiltering; skipping.")
    fileset = filtered_fileset
    ...
    output = iterative_run(
        fileset,
        treename="Events",
        processor_instance=MuonTriggerSFProcessor(year=YEAR),
    )
    # save output accumulator for later use
    output_path = os.path.join(os.getcwd(), f"output/muon_trig_sf_{YEAR}.coffea")
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    print(f"Saving processor output to: {output_path}")
