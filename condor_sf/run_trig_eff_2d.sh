#!/usr/bin/env bash
# run_trig_eff_2d.sh — wrapper to run trig_eff_2d.py with CLI args
...
# Per-run directory: trig_eff_2d_YEAR
RUN_DIR="${SCRIPT_DIR}/trig_eff_2d_${YEAR}"
mkdir -p "${RUN_DIR}"
...
echo ">>> Running 2D trigger efficiency: year=${YEAR}"
if [[ -n "${TEST_MODE}" ]]; then
...
set -x
"${PYTHON_BIN}" "${SCRIPT_DIR}/trig_eff_2d.py" --year "${YEAR}" ${TEST_MODE} 2>&1 | tee "${LOGFILE}"
set +x

echo ">>> Done. Processed both VBF and ggF production modes."
